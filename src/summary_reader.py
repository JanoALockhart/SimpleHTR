...
def plot_summary():
    ...
    word_accuracy = [epoch_summary.word_accuracies for epoch_summary in epochs]
    
    plot_metric("Loss", loss)
    plot_metric("CER", character_error_rate)
    plot_metric("Word Accuracy", word_accuracy)
    ...
    with open("../model/description.txt", 'w') as file:
        ...
        file.write(f"Word Accuracy: {saved_model_word_accuracy*100: .2f}% \n")
